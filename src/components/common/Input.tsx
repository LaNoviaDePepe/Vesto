...
export default function Input({ label, error, disabled, type, ...props }: InputProps) {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            {/* Contenedor dinámico: fila para checkbox, columna para el resto */}
            <div className={`flex ${isCheckbox ? "flex-row items-center gap-3" : "flex-col gap-2"}`}>

                {/* Si es checkbox, el input va PRIMERO */}
                {isCheckbox && (
                    <input
                        type="checkbox"
                ...
                {/* 3. Input de Texto/Password/etc (Va después del label si NO es checkbox) */}
                {!isCheckbox && (
                    <div className="relative group w-full">
                        ...
                        {type === "password" && (
                            <button
                                ...
                                className="absolute right-2 top-1/2 -translate-y-1/2 transition-colors justify-end hover:shadow-none"
                            >
                                {showPassword ? (
