...
interface ModalModificarProps {
  ...
  user: any; // El usuario seleccionado para editar
}
...
export default function ModalModificar({ isOpen, onClose, onConfirm, user }: ModalModificarProps) {
  ...
  useEffect(() => {
    if (user && isOpen) {
      setNombre(user.nombre_apellidos || "");
    }
  }, [user, isOpen]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onConfirm({ nombre_apellidos: nombre }); // Solo enviamos el nombre
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-auxiliary-700/30 backdrop-blur-sm p-4">
      <div className="bg-white p-8 rounded-xl shadow-2xl max-w-md w-full">
        <h3 className="text-2xl font-semibold text-primary-900 mb-6">Modificar Nombre</h3>
